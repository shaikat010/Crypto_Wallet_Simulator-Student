...
from nft import CollectionStorage, AssetStorage, TradeStorage
...
class Trilio:
    ...
    def create_block(self, addresses=None, collections=None, assets=None, trades=None):
        ...
        if len(self.trilio.pending_transactions) >= self.trilio.minimum_transactions:
            ...
            block = Block(datetime.now().timestamp(), transactions, self.trilio.chain[len(self.trilio.chain) - 1].hash,
                          addresses, collections, assets, trades)
